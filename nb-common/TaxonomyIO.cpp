...
bool TaxonomyIO::taxonomy(const std::string& seqId, const std::string& modelName, TaxonomyModel& taxonomyModel) const 
{ 
	...
	if(it != m_seqToTaxonomyMap.end())
	{
		taxonomyModel = it->second;
		return true;
	}

	// try appending model name
	id += "_" + modelName;
	it = m_seqToTaxonomyMap.find(id);
	if(it != m_seqToTaxonomyMap.end())
	{
		taxonomyModel = it->second;
		return true;
	...
	std::cerr << "[Error] Sequence " << seqId << " in model " << modelName << " has no taxonomic information." << std::endl;
